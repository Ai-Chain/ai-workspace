define([
  ...
  function accessPortDialog() {
    ...
    var form = $("<form/>");
    div.append(
      '<label style="width: 50px">Port: </label><input type="number" id="port-input" style="width: 200px" min="1" max="65535"><br>'
    );
    div.append(
      '<br><label style="width: 120px">Get shareable link: </label><input type="checkbox" id="shareable-checkbox">'
    );
    form.appendTo(div);
    return div;
  }
  ...
  function sshInstructionDialog(setup_command) {
    ...
    var div = $("<div/>");
    div.append(
      '<p>SSH provides a powerful set of features that enables you to be more productive with your development tasks as documented <a target="_blank" href="https://github.com/ai-chain/ai-workspace#ssh-access">here</a>. To setup a secure and passwordless SSH connection to this workspace, please execute: </p>'
    );
    div.append("<br>");
    div.append(
      '<textarea readonly="true" style="width: 100%; min-height: 25px; height: 45px" id="ssh-setup-command">' +
        setup_command +
        "</textarea>"
    );
    div.append("<br>");
    div.append(
      '<div style="font-size: 11px; color: #909090;">During the setup process, you will be asked for input, e.g to provide a name for the connection. You can also download, copy and run this script to any other machine to setup an SSH connection to this workspace. This scripts only runs on Mac and Linux, Windows is currently not supported.</div>'
    );
    return div;
  }
  ...
  function installToolDialog(installers) {
    ...
    var div = $("<div/>");
    div.append(
      "<p>The workspace contains a collection of installer scripts for many commonly used development tools or libraries.</p>"
    );
    div.append(
      "<p>1. Please select a tool for further installation instructions:</p>"
    );
    div.append("<br>");

    installer_options =
      "<option disabled selected>Choose Tool Installer</option>";
    ...
    div.append(
      '<select class="form-control" id="install-tool-selection" style="width: 100%; margin-left: 0px;" onchange="(function(e){document.getElementById(\'install-command\').value = e.target.value})(event)">' +
        installer_options +
        "</select>"
    );
    div.append("<br>");
    div.append(
      "<p>2. Run the following command within a workspace terminal to install and start the tool:</p>"
    );
    div.append("<br>");
    div.append(
      '<input readonly="true" style="width: 100%; height: 35px; padding: 7px;" id="install-command" value="">'
    );
    return div;
  }

  //github.com/ai-chain/ai-workspace#ssh-access
  https: function load_ipython_extension() {
    // log to console
    ...
    components.getToolingList(function (tools) {
      ...
      for (var i in tools) {
        ...
        if (tool["url_path"]) {
          ...
          tools_menu_items +=
            '<li> <a id="' +
            tool["id"] +
            '" role="menuitem" tabindex="-1" target="_blank" href="' +
            tool["url_path"] +
            '">' +
            tool["name"] +
            '<span style="display: block; color: #gray; padding: initial; font-size: 12px;">' +
            tool["description"] +
            "</span></a> </li>";
        } else {
          tools_menu_items +=
      ...
      docs_button = '<form action="https://github.com/ai-chain/ai-workspace#features"  target="_blank" style="display: inline-block;margin-left: 2px;"><button class="btn btn-default btn-sm"> <i class="fa-github fa"></i> Docs</button></form>'
      ...
      tools_dropwdown =
        ...
        '<li><div style="display: block;padding: 3px 20px;clear: both;font-weight: 400;line-height: 1.42857143;color: #333333;white-space: nowrap;"><div>Do you like ML Workspace?</div><span style="display: block; color: #gray; padding: initial; font-size: 12px;"><a href="https://github.com/ai-chain/ai-workspace" style="color: #333333;font-weight: 550;" target="_blank">Give us a star</a> or <a href="https://twitter.com/aitooling" style="color: #333333;font-weight: 550;" target="_blank">follow us on Twitter!</a></span></div></li>' +
        " </ul> </div>";        
            ...
            buttons: {
              "Download Script": {
                click: function () {
                  window.open(
                    basePath +
                      "tooling/ssh/setup-script?origin=" +
            ...
            buttons: {
              ...
              "Copy & Open Terminal": {
                ...
                click: function () {
                  ...
                  selecteTool = $(
                    "#install-tool-selection option:selected"
                  ).text();
                  window.open(
                    basePath +
                      "terminals/new/" +
